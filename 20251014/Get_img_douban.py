        ...
        try:
            ...
            for item in movie_items:
                try:
                    ...
                    if title_element:
                        # 我们使用BeautifulSoup库来解析HTML。这里，我们从一个电影项（item）中查找class为'title'的span标签，然后获取其文本内容，并去除两端的空白字符。具体步骤：
                        # 1.找到包含电影标题的span标签（class ="title"）
                        # 2.使用get_text()方法获取标签内的文本
                        # 3.使用strip()方法去除文本两端的空白字符（包括空格、换行符等）
                    ...
                    if img_element and img_element.get('src'):  #确保img_element存在并且有'src'属性。
                        img_url = img_element['src']

                        # 下载图片
                        img_response = requests.get(img_url, headers=headers, timeout=10)   #得到的 img_response.content 就是图片的二进制数据，可以直接保存为图片文件！
                        ...
                        # 我们使用os.path.join来将目录路径和文件名组合成一个完整的文件路径。
                        # 这样做的好处是它会自动使用操作系统的路径分隔符（Windows是反斜杠\，Linux / Mac是斜杠 /），使得代码在不同操作系统上都能正常工作。
                        # 例如，在Windows上：
                        # save_dir = "D:\Typora\Programs_python\20251013"
                        # filename = "肖申克的救赎.jpg"
                        # os.path.join(save_dir, filename)返回"D:\Typora\Programs_python\20251013\肖申克的救赎.jpg"
                        ...
                        with open(filepath, 'wb') as f:
                            f.write(img_response.content)   #得到的 img_response.content 就是图片的二进制数据，可以直接保存为图片文件！
